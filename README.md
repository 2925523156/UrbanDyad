...
|       |-- README.md
|       |-- config.json
|       `-- urban_dyad_static_encoder.ckpt
|-- scripts/
|   |-- pretrain_static.py
|   |-- pretrain_dynamic.py
...
This repository includes the static representation checkpoint:

```text
checkpoints/static/urban_dyad_static_encoder.ckpt
```

The checkpoint is larger than the normal GitHub file limit and should be uploaded with
Git LFS. Other model checkpoints, downstream predictors, learned embeddings, and large
intermediate arrays are not included.
...
records, Baidu street-view images, and Amap POI records.
...
UrbanDyad. Since raw data and dynamic model weights are not redistributed, exact
reproduction requires access to the corresponding urban datasets or compatible
alternatives.
