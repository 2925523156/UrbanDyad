...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Predict hourly order volume from hourly dynamic embeddings.")
    parser.add_argument(
        "--input-csv",
        type=str,
        default=str(Path("data") / "sample" / "dynamic_embeddings_with_hourly_demand.csv"),
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=str(Path("outputs") / "hourly_demand"),
    )
    parser.add_argument("--target-col", type=str, default="order_volume")
    parser.add_argument("--date-col", type=str, default="date")
    parser.add_argument("--hour-col", type=str, default="hour")
    ...
    parser.add_argument("--enforce-continuous-hours", action="store_true")
    parser.add_argument("--split-mode", type=str, choices=["date", "spatiotemporal"], default="date")
    parser.add_argument("--test-time-ratio", type=float, default=0.3)
    parser.add_argument("--val-time-ratio", type=float, default=0.2)
...
def filter_all_zero_regions(
    ...
    nonzero_counts = df[target_col].fillna(0.0).abs().gt(1e-8).groupby(df[region_col], sort=False).sum()
    dropped_regions = nonzero_counts[nonzero_counts.eq(0)].index.astype(str)
    filtered = df.loc[~df[region_col].astype(str).isin(dropped_regions)].copy()
    stats = {
        "raw_row_count": raw_row_count,
        "raw_region_count": raw_region_count,
        "dropped_all_zero_regions": int(len(dropped_regions)),
    ...
    print(
        ...
        f"dropped_all_zero_regions={stats['dropped_all_zero_regions']} "
...
def load_table(args: argparse.Namespace) -> tuple[pd.DataFrame, list[str], dict[str, int]]:
    ...
    required_cols = [args.region_col, "time_index", args.date_col, args.hour_col, args.target_col]
    missing = [col for col in required_cols if col not in df.columns]
    if missing:
        raise ValueError(f"Missing required columns: {missing}")

    df, filter_stats = filter_all_zero_regions(df, args.region_col, args.target_col)
    df = df.copy()
    ...
    df["parsed_time"] = pd.to_datetime(
        df[args.date_col] + df[args.hour_col].map(lambda value: f"{value:02d}"),
        format="%Y%m%d%H",
        errors="raise",
    )
    df["time_index"] = df["time_index"].astype(int)
    df["time_label"] = df["parsed_time"].dt.strftime("%Y%m%d_%H")
    df = df.dropna(subset=feature_cols + [args.target_col, args.date_col, args.hour_col, "time_index"]).copy()
...
def is_continuous_hourly_window(parsed_times: pd.Series) -> bool:
    diffs = parsed_times.diff().dropna()
...
def build_forecast_table(
    ...
    for region_id, group in grouped:
        ...
        parsed_times = group["parsed_time"]
        for input_pos in range(embedding_history_length - 1, len(group)):
            target_pos = input_pos + forecast_horizon
            if target_pos >= len(group):
                continue

            if enforce_continuous_hours:
                window_times = parsed_times.iloc[input_pos - embedding_history_length + 1 : target_pos + 1]
                if not is_continuous_hourly_window(window_times):
                    continue
            ...
            rows.append(
                {
                    ...
                    "input_hour": int(input_row[hour_col]),
                    "target_time_index": int(target_row["time_index"]),
                    "target_date": target_row[date_col],
                    "target_hour": int(target_row[hour_col]),
                    "target_time_label": target_row["time_label"],
    ...
    if not rows:
        raise ValueError(
            "No supervised samples were built. Check history/horizon settings or disable --enforce-continuous-hours."
        )
...
def split_by_time(
    df: pd.DataFrame,
    time_col: str,
    test_time_ratio: float,
    val_time_ratio: float,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    time_labels = np.array(sorted(df[time_col].astype(str).unique()))
    if len(time_labels) < 3:
        raise ValueError("Need at least 3 unique target hours for train/val/test splitting.")

    test_count = max(1, int(round(len(time_labels) * test_time_ratio)))
    if len(time_labels) - test_count < 2:
        test_count = max(1, len(time_labels) - 2)
    train_val_times = time_labels[:-test_count]
    test_times = time_labels[-test_count:]

    val_count = max(1, int(round(len(train_val_times) * val_time_ratio)))
    if len(train_val_times) - val_count < 1:
        val_count = max(1, len(train_val_times) - 1)
    train_times = train_val_times[:-val_count]
    val_times = train_val_times[-val_count:]

    return (
        df[time_col].isin(train_times).to_numpy(),
        df[time_col].isin(val_times).to_numpy(),
        df[time_col].isin(test_times).to_numpy(),
    )
...
def split_by_spatiotemporal(
    ...
    time_labels = np.array(sorted(df[time_col].astype(str).unique()))
    if len(time_labels) < 3:
        raise ValueError("Need at least 3 unique target hours for spatiotemporal splitting.")
    ...
    if len(regions) < 2:
        raise ValueError("Need at least 2 regions for spatiotemporal splitting.")
    ...
    test_regions = set(regions[:test_region_count].tolist())
    seen_regions = set(regions[test_region_count:].tolist())

    train_mask = df[time_col].isin(train_times).to_numpy()
    val_mask = (df[time_col].isin(val_times) & df[region_col].isin(seen_regions)).to_numpy()
    test_mask = (df[time_col].isin(test_times) & df[region_col].isin(test_regions)).to_numpy()

    if not train_mask.any() or not val_mask.any() or not test_mask.any():
        raise ValueError(
            "Spatiotemporal split produced an empty train/val/test set. "
            "Adjust test/val ratios or region count."
        )
...
def save_prediction_plots(df_out: pd.DataFrame, target_col: str, output_dir: Path) -> None:
    plot_df = df_out.sort_values(["target_time_index", "region_id"]).reset_index(drop=True)
    ...
    tick_positions = plot_df.groupby("target_time_label", sort=True).head(1).index.to_numpy()
    tick_labels = plot_df.loc[tick_positions, "target_time_label"].astype(str).to_numpy()
    plt.xticks(tick_positions, tick_labels, rotation=45, ha="right")
    plt.xlabel("Target hour, then district order")
    plt.ylabel(target_col)
    plt.title("Hourly order volume prediction curve: all district-hour samples")
    plt.legend()
    plt.tight_layout()
    ...
    hourly_mean = (
        plot_df.groupby("target_time_label", as_index=False)[[target_col, "prediction"]]
        .mean()
        .sort_values("target_time_label")
    )
    plt.figure(figsize=(14, 5))
    plt.plot(hourly_mean["target_time_label"], hourly_mean[target_col], marker="o", label="True hourly mean")
    plt.plot(
        hourly_mean["target_time_label"],
        hourly_mean["prediction"],
        marker="o",
        label="Predicted hourly mean",
    )
    plt.xlabel("Target hour")
    plt.ylabel(target_col)
    plt.title("Hourly mean order volume prediction curve")
    plt.xticks(rotation=45, ha="right")
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / "prediction_curve_hourly_mean.png", dpi=200)
    plt.close()
...
def main() -> None:
    ...
    df, feature_cols = build_forecast_table(
        ...
        enforce_continuous_hours=args.enforce_continuous_hours,
    )

    if args.split_mode == "date":
        train_mask, val_mask, test_mask = split_by_time(
            df=df,
            time_col="target_time_label",
            test_time_ratio=args.test_time_ratio,
            val_time_ratio=args.val_time_ratio,
        )
    else:
        ...
        metrics_rows.append(
            {
                ...
                "target_hours": int(df.loc[mask, "target_time_label"].nunique()),
    ...
    metrics_rows.append(
        {
            ...
            "target_hours": int(df_out.loc[used_mask, "target_time_label"].nunique()),
    ...
    save_data_filter_summary(
        output_dir,
        {
            **data_filter_stats,
            ...
            "supervised_target_hour_count": int(df["target_time_label"].astype(str).nunique()),
        },
    )
    ...
    print(
        ...
        f"continuous_hours={args.enforce_continuous_hours}",
        flush=True,
    )
