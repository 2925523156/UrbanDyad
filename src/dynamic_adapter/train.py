...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train the dynamic region representation model.")
    parser.add_argument("--static-embeddings", type=str, default="")
    parser.add_argument("--static-district-ids", type=str, default="", help=argparse.SUPPRESS)
    ...
    parser.add_argument("--district-ids", type=str, default="", help=argparse.SUPPRESS)
...
def resolve_output_dir(args: argparse.Namespace) -> Path:
    ...
    hourly_region_ids_path = args.region_ids or args.district_ids or args.cell_ids
    s2_like_run = bool(
        args.cell_ids
        or (hourly_region_ids_path and "cell_ids" in Path(hourly_region_ids_path).name.lower())
        or (args.hourly_state and "cell_ids" in str(args.hourly_state))
        or (args.hourly_state and "weekavg" in str(args.hourly_state))
    )
    default_prefix = "dynamic_train_hourly_s2_od" if s2_like_run else "dynamic_train_hourly"
...
def main() -> None:
    ...
    if args.use_mock_data:
        ...
        if any(not value for value in required_paths):
            raise ValueError(
                "Hourly-state training requires --static-embeddings, one of "
                "--static-region-ids/--static-district-ids/--static-cell-ids, "
                "--hourly-state, one of --region-ids/--district-ids/--cell-ids, "
                "--time-context, and --time-index."
            )
        ...
        args.dynamic_dim = int(bundle.hourly_state.shape[2])
        ...
        if any(not value for value in required_paths):
            raise ValueError(
                "Real-data training requires --static-embeddings, one of "
                "--static-region-ids/--static-district-ids/--static-cell-ids, "
    ...
    print(
        f"Hourly dynamic training samples={bundle.num_samples} "
        ...
        f"context_mode=time_context_only",
        flush=True,
    )
    if args.resume_from:
        print(
            f"Resuming hourly training from: {args.resume_from} | "
    ...
    print(f"Dynamic training finished. Outputs saved to: {output_dir}")
