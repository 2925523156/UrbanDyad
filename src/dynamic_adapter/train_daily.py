...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train the daily dynamic region/cell representation model.")
    ...
    parser.add_argument(
        "--same-day-of-month-positives",
        action="store_true",
        help="Use same district and same day-of-month across different months as daily periodic positives.",
    )
    parser.add_argument(
        "--same-day-of-week-positives",
        action="store_true",
        default=True,
        help="Use same district and same weekday across sampled days as daily periodic positives.",
    )
...
def resolve_output_dir(args: argparse.Namespace) -> Path:
    ...
    daily_region_ids_path = args.region_ids or args.district_ids or args.cell_ids
    s2_like_run = bool(
        args.cell_ids
        or (daily_region_ids_path and "cell_ids" in Path(daily_region_ids_path).name.lower())
        or (args.daily_state and "daily_dynamic_inputs_s2" in str(args.daily_state))
    )
    default_prefix = "dynamic_train_daily_s2_od" if s2_like_run else "dynamic_train_daily"
...
def main() -> None:
    ...
    daily_region_ids_path = args.district_ids or args.region_ids or args.cell_ids
    ...
    if args.use_mock_data:
        ...
        required_paths = [
            ...
            args.daily_state,
            daily_region_ids_path,
            args.day_context,
            args.date_index,
        ]
        if any(not value for value in required_paths):
            raise ValueError(
                "Daily-state training requires --static-embeddings, one of "
        ...
        bundle = load_daily_dynamic_feature_bundle(
            ...
            daily_region_ids_path=Path(daily_region_ids_path),
            ...
            same_day_of_month_positives=args.same_day_of_month_positives,
            same_day_of_week_positives=args.same_day_of_week_positives,
        )
        args.static_dim = int(bundle.static_embeddings.shape[1])
        args.dynamic_dim = int(bundle.daily_state.shape[2])
        args.time_dim = int(bundle.day_context.shape[1])
    ...
    print(
        f"Daily dynamic training samples={bundle.num_samples} "
        ...
        f"same_day_of_month_positives={args.same_day_of_month_positives} "
        f"same_day_of_week_positives={args.same_day_of_week_positives} "
        f"dynamic_window_mode=history_plus_current_state "
        f"context_mode=day_context_only",
        flush=True,
    )
    if args.resume_from:
        print(
            f"Resuming daily training from: {args.resume_from} | "
    ...
    print(f"Daily dynamic training finished. Outputs saved to: {output_dir}")
